...
public class JdbcSongRepository implements SongRepository{
	...
	public Song findById(Integer id) {
		...
		return jdbcTemplate.queryForObject(sql, songsMapper, id);
	}
	...
	public List<Song> findAllContaining(String text){
		String sql = "select * from songs where lower(text) like lower(?)";
		return jdbcTemplate.query(sql, songsMapper, "%" + text + "%");
	}
	
	@Override
	public List<String> findAllArtists(){
		String sql = "select distinct lower(artist) from songs";
