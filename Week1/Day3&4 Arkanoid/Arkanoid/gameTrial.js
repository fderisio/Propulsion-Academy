function Game() {
	...
	for (let i = 0; i < 6; i++) { // rows
		...
		for (let j = 0; j < blockColumns; j++) { // columns
			...
			// creates a block
			var block = new Block([spaceX + j * (width + brickPadding), spaceY + i * (height + brickPadding)], [j, i], height, width, randomColor);  
...
Game.prototype.play = function() {
	...
	setInterval(function callMeMaybe() {
		...
		for (let i = 0; i < this.blocks.length; i++) {
			...
			this.ball.collidesBrick(this.blocks[i].blockColRow, this.blocks[i].blockPosition[0]);
		};
